...
class BaseModelGenerator(object):
    ...
    def __init__(self, params):
        ...
        self.def_string_pk1 = '    %(c_name)s = Column(%(type)s, nullable=%(nullable)s,unique=%(uniq)s,default=%(default)s)'
    ...
    def generate_class_string(self, table_name):
        ...
        for i in auto_table.columns:
            ...
            coltype = i.type
            try:
                c_length = coltype.length
            except:
                c_length = ""

            # print dir(coltype)
            v_name = coltype.__visit_name__
            coltype = "types.%s(%s)" % (v_name, c_length)
            if v_name == "TINYINT":
